...
from construct import (
    ...
    ULInt8, ULInt16, ULInt32, ULInt64, SLInt16,
...
class ELFStructs(object):
    ...
    def __init__(self, little_endian=True, elfclass=32):
        assert elfclass == 32 or elfclass == 64 or elfclass == 16
    ...
    def _create_structs(self):
        if self.little_endian:
            ...
            self.Elf_word = ULInt16 if self.elfclass == 16 else ULInt32
            
            if self.elfclass == 16:
                self.Elf_addr = ULInt16
            elif self.elfclass == 32:
                self.Elf_addr = ULInt32
            else:
                self.Elf_addr = ULInt64
            
            self.Elf_offset = self.Elf_addr
            
            if self.elfclass == 16:
                self.Elf_xword = ULInt16
            elif self.elfclass == 32:
                self.Elf_xword = ULInt32
            else:
                self.Elf_xword = ULInt64
            
            if self.elfclass == 16:
                self.Elf_sxword = SLInt16
            elif self.elfclass == 32:
                self.Elf_sxword = SLInt32
            else:
            ...
            if self.elfclass == 16:
                self.Elf_addr = UBInt16
            elif self.elfclass == 32:
                self.Elf_addr = UBInt32
            else:
                self.Elf_addr = UBInt64
            self.Elf_offset = self.Elf_addr
            self.Elf_xword = UBInt64 if self.elfclass == 64 else UBInt32
            self.Elf_sxword = SBInt64 if self.elfclass == 64 else SBInt32
    ...
    def _create_phdr(self):
        if self.elfclass == 32 or self.elfclass == 16:
